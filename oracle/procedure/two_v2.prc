...
begin
  ...
  loop
    ...
    dbms_output.put_line('设备编号：'||dvs.deviceid);
    dbms_output.put_line('设备所属：'||dvs.clientid);
    dbms_output.put_line('设备类型：'||dvs.type);
    dbms_output.put_line('设备余额：'||dvs.balance);
  end loop;
  close cur_d;
...
exception
  when not_found_exception then
    dbms_output.put_line('输入错误，编号设备不存在');
  res:='失败';
end two_v2;
/
