...
float linefit(Point l_point[],int n_point)
{
	...
	{
		av_x+=l_point[i].GetX()/n_point;
		av_y+=l_point[i].GetY()/n_point;
	}
	for(i=0;i<n_point;i++)
	{
		L_xx+=(l_point[i].GetX()-av_x)*(l_point[i].GetX()-av_x);
		L_yy+=(l_point[i].GetY()-av_y)*(l_point[i].GetY()-av_y);
		L_xy+=(l_point[i].GetX()-av_x)*(l_point[i].GetY()-av_y);
