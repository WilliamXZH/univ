	...
	public void onReceive(Context context, Intent intent)
	{
		...
		} else if (WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION.equals(action))
		{
			...
			if (manager != null)
			{
				...
				//这是个异步的调用，并且，调用行为是通过PeerListListener.onPeersAvailable()上的一个回调函数来通知的。
				...
				.equals(action))
		{
			...
			if (networkInfo.isConnected())
			{
				...
				//这是一个异步的调用，结果会传给作为参数的连接信息监听器，
				//调用行为是通过ConnectionInfoListener.onConnectionInfoAvailable()上的一个回调函数来通知的。
