...
public class FTPClient {
	...
	public synchronized void listFiles(String serverPath) throws IOException{
		//writer.write("cwd "+serverPath+"/r/n");
	...
	public synchronized boolean upload(String lfilepath,String serverPath) throws IOException{
		...
		//writer.println("cwd "+sererPath+"/r/n");
		/*writer.println("cwd "+serverPath);
		writer.flush();
		System.out.println(reader.readLine());*/
		...
		if(closing >0){
			...
			try{
				...
				throw new IOException("SimpleFTP received bad date link information:"+response);
		...
		output.close();
		input.close();
		System.out.println("finish file's upload.");
...
//		writer.flush();
		...
		if(closing>0){
			String dataLink=response.substring(opening+1,closing);
			String []tokenizer=dataLink.split(",");
			try{
	...
	public static void main(String args[]) throws IOException{
		...
		fr.listFiles("aa");
		boolean b=fr.download("aa.txt", "aa","d:/" );
		System.out.println("download file:"+b);
		b=fr.upload("d:/_aa.txt", "f:/aa");
		System.out.println("upload file:"+b);
		fr.disconnect();
	}
}
