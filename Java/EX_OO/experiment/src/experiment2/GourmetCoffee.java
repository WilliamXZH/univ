...
public class GourmetCoffee  {
	...
	public void displayOrdersSold() {
		...
		if (numOrders != 0) {
			/*int orderNumber = 0;
			Iterator<Order> sale=sales.iterator();
			while(sale.hasNext()){
				Order order=sale.next();
				stdOut.println("Order " + orderNumber++);
				Iterator<OrderItem> orderi=order.iterator();
				while(orderi.hasNext()){
						stdOut.println("   " + orderi.next().toString());
				}
				stdOut.println("   Total: " +
	...
	public void displayNumberOfOrders(Product product) {
		...
		for(Order order:sales){
			...
			/*
			 *也可以借助order类中的getItem方法写成以下更简单的形式:
			 *if(order.getItem(product)!=null)number++;
			 */
