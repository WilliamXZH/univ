 ...
 * @see DataField
 */
public class GourmetCoffeeGUI2 extends JPanel {
	...
	static private int CATALOG_LIST_ROWS = 11;
	...
	public static void  main(String[]  args) throws IOException {
		...
		try {
			...
			frame.setContentPane(new GourmetCoffeeGUI2(catalog));
	 ...
	 */
	public  GourmetCoffeeGUI2(Catalog initialCatalog) {

		// create the components
		catalogList = new JList<Product>();
	...
	private JPanel getDataFieldsPanel(ArrayList<DataField> dataFields) {
		...
		left.setPreferredSize(new Dimension((int)(0.2*productPanel.getWidth()),(int)(0.80*productPanel.getHeight())));
		right.setPreferredSize(new Dimension((int)(0.6*productPanel.getWidth()),(int)(0.80*productPanel.getHeight())));
		...
		for(DataField datafield:dataFields){
			...
			right.add(textfield,BorderLayout.WEST);
		}
	...
	class AddModifyListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			String code = (String) catalogList.getSelectedValue();
			String qt=quantityTextField.getText();
			int quantity;
			try{
				quantity=Integer.parseInt(qt);
			}catch(Exception e){
				statusTextArea.setText("quantuty is invalid!");
				return;
			}
			Product product = catalog.getProduct(code);
	...
	class RemoveListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			if(currentOrder.getNumberOfItems()==0){
				statusTextArea.setText("orderList is empty.");
			}
			else if(((OrderItem)orderList.getSelectedValue())==null){
				totalTextField.setText("Please selected an item from the orderList.");
			}
			else{
			...
			if (sales.getNumberOfOrders() == 0) {
				...
				if (i != 0) {
					...
					try {
						...
						e.printStackTrace();
						statusTextArea.setText("IO Exception!!");
					}
				}
			}
			/* PLACE YOUR CODE HERE */
