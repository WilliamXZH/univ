...
public class GourmetCoffeeGUI extends JPanel {
	...
	private JPanel getDataFieldsPanel(ArrayList<DataField> dataFields) {
		...
		left.setPreferredSize(new Dimension(100,(int)(0.80*productPanel.getHeight())));
		right.setPreferredSize(new Dimension((int)(0.5*productPanel.getWidth()),(int)(0.8*productPanel.getHeight())));
	...
	class AddModifyListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			/*double totalcost=Double.parseDouble(totalTextField.getText());
			int quantity=Integer.valueOf(quantityTextField.getText());
			String code = (String) catalogList.getSelectedValue();
			Product product = catalog.getProduct(code);
			orderList.add(number,product.getDataFields(), quantity);*/
			String code = (String) catalogList.getSelectedValue();
			if(code==null){
	...
	class RemoveListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
				...
				if (currentOrder.getNumberOfItems() == 0) {
					statusTextArea.setText("The currentOrder is empty.");
				} else {
					...
					if (orderItem != null) {
						...
						statusTextArea
								.setText("The user has not selected an item in the order.");
					}
				}
