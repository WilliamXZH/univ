...
<link href="css/ChangeEmail.css" rel="stylesheet" type="text/css"
	media="all" />
...
<script type="text/javascript">
			...
			function submitAllMessageOfModifyMail(){
				...
				$.ajax({
					...
					success:function(result){
						console.log(result);
						window.location.href="<%=basePath%>userweb/first";
			},
				...
				<div class="row">
					...
					<div class="col-lg-6 col-md-8 col-sm-9">
						...
						<ul>
							...
							<li><a href="<%=basePath %>userweb/NomalQuestion" >常见问题</a></li>
						...
						<div class="change_new_pssword">
							...
							<form id="changeNewPass">
								<div class="My_password">
									<label>登录密码：</label><input class="pass_style" name="myOldPass"
										id="password2" type="password" />
								</div>
								<div class="My_email">
									<label>新邮箱&nbsp;&nbsp;&nbsp;：</label><input class="pass_style2"
										type="text" name="myNewEmail" id="mail2"
										placeholder="请输入新邮箱地址" />
								</div>
								<div class="My_check_number">
									<label>验证码&nbsp;&nbsp;&nbsp;：</label> <input type="text"
										name="identNumber" class="check_number" id="mailVerifyCode" />
									<input type="submit" name="my_new_check"
										class="get_checkNumber" value="获得验证码"
										onclick="sendVerificationCodeToMailBox()" />
								</div>
								<input type="submit" class="new_pass_submit" value="提交"
									onclick="submitAllMessageOfModifyMail()" />
							</form>
