...
function initPage(res){
	...
	if(res.length==0){
		...
		document.getElementById("pageGroup").style.width=100+res.length*30+"px";
	}
	...
	document.getElementById("pageGroup").style.display="block";
	...
	page_ul.innerHTML="";
	for(var j=1;j<=pageNoCount&&j<=totalPage&&totalPage!=1;j++){
...
/*	var tmp=[];
	...
	pagePlay();
	showData(totalData.slice(start,end));
}
