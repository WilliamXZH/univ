...
public class TicketPurchaseController {
	...
	public ModelAndView ticketPurchase(String info,HttpSession session){
		...
		for (TrainVO trainVO : trips) {
			if(trainVO.getId()==tripTrainId){
				...
				session.setAttribute("trainInfo", triptrainJsonobject);
				break;
			}
		}
	...
	public void addOrder(HttpServletRequest request,HttpServletResponse response,HttpSession session) throws IOException, ParseException{
        ...
        while(it.hasNext()){
        	...
        	newOrder.setCost((Integer)orderItem.get("cost"));
        	...
        	//初始化resultList...
            resultList.add(new resultOfSeatSelection(newOrderId,newOrder.getMemberName(),
            ...
            session.setAttribute("bdDate", orderItem.get("departTime"));
        	seatIndex++;
        }
        ...
        session.setAttribute("passengerData", jsonArray);
        session.setAttribute("trainName",trainInfo.get("trainName"));
        
        
	}
	@RequestMapping("purchase/payForOrder")
	public void orderConfirm(HttpServletRequest request,HttpServletResponse response,HttpSession session){
		String payMethod = request.getParameter("payMethod");
		Integer ifDeliver = Integer.parseInt(request.getParameter("ifDeliver"));
		System.out.println("【payForOrder】-->parameter:payMethod("+payMethod+"),ifDeliver("+ifDeliver+")");
		JSONArray passengerDataList = (JSONArray) session.getAttribute("passengerData");
		Iterator<Object> it = passengerDataList.iterator();
		while(it.hasNext()){
			JSONObject pdItem = (JSONObject) it.next();
			int orderId = (int) pdItem.get("orderId");
			System.out.println("orderId:"+pdItem.get("orderId")); 
			if(orderService.payForOrder(orderId,payMethod, ifDeliver)){
				System.out.println("orderId:"+pdItem.get("orderId")+"修改成功");
			}
		}
