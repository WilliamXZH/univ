...
public class OrderController {
	...
	public void selectOrder(@ModelAttribute OrderQueryConditionVO orderConditon,HttpServletResponse response) throws ParseException, IOException{
		...
		JSONArray jsonArray;
		System.out.println("id:"+orderConditon.getId());
		if(orderConditon.getWay().equals("1")){
			if(orderConditon.getCondition()!=null && orderConditon.getCondition()!=""){
				list = orderService.selectOrder(orderConditon.getId(),orderConditon.getCondition(),
						orderConditon.getStart(),orderConditon.getEnd());
			}else{
				list = orderService.selectOrder2(orderConditon.getId(),
						orderConditon.getStart(), orderConditon.getEnd());
			}
		}else{
			if(orderConditon.getCondition()!=null && orderConditon.getCondition()!=""){
				list = orderService.selectOrder3(orderConditon.getId(),orderConditon.getCondition(),
						orderConditon.getStart(),orderConditon.getEnd());
			}else{
				list = orderService.selectOrder4(orderConditon.getId(),
						orderConditon.getStart(),orderConditon.getEnd());
			}
		}
